RSpec.describe 'allow method review' do
  it 'can customize return value for methods on doubles' do
    calculator = double
    allow(calculator).to receive(:add).and_return(4)

    expect(calculator.add).to eq(4)
    expect(calculator.add(3)).to eq(4)
    expect(calculator.add(-2, 4)).to eq(4)
    expect(calculator.add('hello world')).to eq(4)
  end

  it 'can stub one or more methods on a real object' do
    arr = [1, 2, 3]
    allow(arr).to receive(:sum).and_return(3)
    expect(arr.sum).to eq(3)
